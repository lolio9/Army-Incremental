...
function startMission(name, rewardMoney, rewardXP, duration) {
    if (activeMissions[name]) {
        alert(`${name} is already in progress!`);
        return;
    }
    const endTime = new Date().getTime() + duration * 60000;
    activeMissions[name] = { rewardMoney, rewardXP, endTime };

    const modal = document.getElementById('missionModal');
    modal.style.display = 'block';
    document.getElementById('missionTitle').innerText = name;
    document.getElementById('missionDescription').innerText = `Mission ${name} is in progress.`;
    updateCountdown(name, endTime);
